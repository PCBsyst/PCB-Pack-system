...
export function ApplicationDetail({ application, candidate, linkedJobs, invoices }: { application: CertificationApplication; candidate: Candidate; linkedJobs: Job[]; invoices: Invoice[] }) {
  ...
  useEffect(() => { const stored = window.localStorage.getItem(storageKey); if (stored) { const initial = makeInitial(application, linkedJobs); const saved = JSON.parse(stored) as Partial<DemoState>; setDemo({ ...initial, ...saved, review: { ...initial.review, ...saved.review }, englishText: { ...initial.englishText, ...saved.englishText }, assessment: saved.assessment ?? initial.assessment, panelMembers: saved.panelMembers ?? initial.panelMembers }); } }, [application, linkedJobs, storageKey]);
  ...
  const generate = () => { if (!languages.KR && !languages.EN) { setNotice("생성할 언어를 하나 이상 선택해 주세요."); return; } setDemo((current) => ({ ...current, stage: "COMPLETED", generated: true })); setNotice("Job별 국문·영문 기록 패키지가 생성되었습니다. Word·PDF·ZIP 다운로드를 시험해 보세요."); };
  const downloadZip = () => {
...
function DocumentButtons({ document, enabled, setNotice }: { document: ReturnType<typeof buildDocuments>[number]; enabled: boolean; setNotice: (message: string) => void }) { if (!enabled) return <span className="text-xs text-slate-400">생성 제외</span>; return <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => downloadWord(document.fileName, document.html)}><Download/>Word</Button><Button size="sm" variant="outline" onClick={() => { try { printAsPdf(document.title, document.html); } catch { setNotice("PDF 창이 차단되었습니다. 이 사이트의 팝업을 허용하세요."); } }}><Printer/>PDF</Button></div>; }
