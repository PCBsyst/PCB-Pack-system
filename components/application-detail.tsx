...
import { buildDocuments, createZip, deliveryDocumentRows, downloadBlob, downloadDecisionReportDocx, downloadDeliveryConfirmationDocx, downloadWord, printAsPdf, type AssessmentResult, type DeliveryDocumentKey, type DemoAssessment, type DemoCertificate, type DemoDecision, type DemoDeliveryDocuments, type DemoEnglishText, type DemoPanelMember, type DemoReview, type DocumentApplicability, type DocumentLanguage } from "@/lib/prototype-package";
...
function makeInitial(application: CertificationApplication, jobs: Job[]): DemoState {
  ...
  return {
    ...
    deliveryDocuments: Object.fromEntries(jobs.map((job) => [job.id, Object.fromEntries(deliveryDocumentRows.map(({ key }) => {
      ...
      const applicability: DocumentApplicability = ["auditLog", "examNotice", "examAnswers", "survey"].includes(key) ? "CONDITIONAL" : "REQUIRED";
...
export function ApplicationDetail({ application, candidate, linkedJobs, invoices }: { application: CertificationApplication; candidate: Candidate; linkedJobs: Job[]; invoices: Invoice[] }) {
  ...
  return <div className="space-y-5">
    ...
    {active === "서류검토" && <div className="space-y-5"><Section title="자격요건 및 제출문서 검토" description="후보자 신청정보와 제출 증빙을 기준으로 실무자가 항목별 결과를 직접 기록합니다."><div className="grid gap-3 sm:grid-cols-2">{["교육요건", "학력요건", "업무경력요건", "심사·실무경력요건"].map((item) => <div key={item} className="rounded-md border p-4"><p className="text-sm font-semibold">{item}</p><div className="mt-3 flex gap-4 text-sm">{["충족", "미충족", "해당없음"].map((value, index) => <label key={value}><input type="radio" name={item} defaultChecked={index === 0}/> {value}</label>)}</div></div>)}</div></Section>
    ...
    {active === "Job·패키지" && <Section title="Job별 인증정보 및 기록 패키지" description="화면은 한글로 운영하며, 기록 문서는 국문·영문으로 각각 생성합니다.">
      <DeliveryDocumentChecklist jobs={linkedJobs} records={demo.deliveryDocuments} setDemo={setDemo}/>
      <div className="mb-5 flex flex-wrap items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 p-4"><div className="mr-auto"><p className="text-sm font-semibold text-emerald-950">기업 양식 DOCX 시험 생성</p><p className="mt-1 text-xs text-emerald-800">제공된 FGPC-012-03 영문 양식의 머리글·바닥글·워터마크를 유지합니다.</p></div>{linkedJobs.map((job) => <Button key={`delivery-download-${job.id}`} size="sm" variant="outline" onClick={async () => { try { await downloadDeliveryConfirmationDocx(packageContext, job); setNotice(`${job.jobNo} 문서전달확인서 DOCX를 생성했습니다.`); } catch { setNotice("문서전달확인서 DOCX 생성에 실패했습니다."); } }}><Download/>{job.jobNo} 영문 Word</Button>)}</div>
...
function DeliveryDocumentChecklist({ jobs, records, setDemo }: { jobs: Job[]; records: DemoDeliveryDocuments; setDemo: React.Dispatch<React.SetStateAction<DemoState>> }) {
  const labels: Record<DocumentApplicability, string> = { REQUIRED: "필수", CONDITIONAL: "해당 시 필수", NOT_APPLICABLE: "해당 없음" };
  return <div className="mb-5 space-y-4">{jobs.map((job) => <div key={`delivery-${job.id}`} className="rounded-lg border">
    <div className="border-b bg-slate-50 px-4 py-3"><p className="text-sm font-semibold">{job.jobNo} · 문서전달확인서 기록 목록</p><p className="mt-1 text-xs text-slate-500">분야·등급에 따라 적용 여부를 선택합니다. 날짜는 아직 서로 연동하지 않습니다.</p></div>
    <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead className="text-xs text-slate-500"><tr><th className="px-3 py-3 text-left">Form</th><th className="px-3 py-3 text-left">문서</th><th className="px-3 py-3 text-left">적용 구분</th><th className="px-3 py-3 text-center">확인</th><th className="px-3 py-3 text-left">기록일</th><th className="px-3 py-3 text-left">비고</th></tr></thead>
      <tbody className="divide-y">{deliveryDocumentRows.map((row) => { const record = records[job.id]?.[row.key]; const disabled = record?.applicability === "NOT_APPLICABLE"; return <tr key={row.key} className={disabled ? "bg-slate-50 text-slate-400" : ""}>
        <td className="px-3 py-2 text-xs">{row.form}</td><td className="px-3 py-2 font-medium">{row.document}</td>
        <td className="px-3 py-2"><select className={controlClass} value={record?.applicability ?? "REQUIRED"} onChange={(event) => { const applicability = event.target.value as DocumentApplicability; changeDeliveryDocument(job.id, row.key, { applicability, ...(applicability === "NOT_APPLICABLE" ? { received: false, date: "" } : {}) }, setDemo); }}>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></td>
...
function changeDeliveryDocument(jobId: string, key: DeliveryDocumentKey, patch: Partial<DemoDeliveryDocuments[string][DeliveryDocumentKey]>, setDemo: React.Dispatch<React.SetStateAction<DemoState>>) { setDemo((current) => ({ ...current, deliveryDocuments: { ...current.deliveryDocuments, [jobId]: { ...current.deliveryDocuments[jobId], [key]: { ...current.deliveryDocuments[jobId][key], ...patch } } } })); }
...
function DocumentButtons({ document, enabled, setNotice, onWord }: { document: ReturnType<typeof buildDocuments>[number]; enabled: boolean; setNotice: (message: string) => void; onWord?: () => void | Promise<void> }) { if (!enabled) return <span className="text-xs text-slate-400">생성 제외</span>; return <div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => onWord ? void onWord() : downloadWord(document.fileName, document.html)}><Download/>Word</Button><Button size="sm" variant="outline" onClick={() => { try { printAsPdf(document.title, document.html); } catch { setNotice("PDF 창이 차단되었습니다. 이 사이트의 팝업을 허용하세요."); } }}><Printer/>PDF</Button></div>; }
