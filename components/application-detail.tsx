...
function makeInitial(application: CertificationApplication, jobs: Job[]): DemoState {
  ...
  return {
    ...
    englishText: { reviewComment: isLeeRenewal ? "Renewal application documents and continued certification requirements were verified." : "", panelComments: Object.fromEntries(panelRoster.map((name, index) => [name, isLeeRenewal && index < 2 ? "Renewal requirements verified." : ""])), decisionComments: Object.fromEntries(jobs.map((job) => [job.id, isLeeRenewal ? "Renewal reapproved." : ""])) },
...
function changeEnglishText(field: "reviewComment", value: string, setDemo: React.Dispatch<React.SetStateAction<DemoState>>) { setDemo((current) => ({ ...current, englishText: { ...current.englishText, [field]: value } })); }
